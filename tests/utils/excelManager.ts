...
export async function readExcelCell(file: string, sheetName: string, cellAddress: string): (string | number) {
    const workbook = new ExcelJS.Workbook();
...
export async function readExcelRow(file: string, sheetName: string, rowNumber: number): (string | number)[] {
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.readFile(file);
    const sheet = workbook.getWorksheet(sheetName);
    const row = sheet.getRow(rowNumber);
